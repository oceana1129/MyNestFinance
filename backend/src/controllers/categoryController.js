...
export async function createCategory(req, res) {
    try {
        ...
        res.status(201).json({message: "Budget Category created successfully!", savedCategory})
    } catch (err) {
        ...
        if (err.code === 11000) {
            return res.status(409).json({
                message:
                "A category already exists with this name for this budget.",
            });
        }
...
export async function getCategoryById(req, res) {
    try {
        ...
        res.status(200).json({message: `Category with id ${req.params.id} found`, category});
    } catch (err) {
...
export async function updateCategory(req, res) {
    try {
        ...
        if (err.code === 11000) {
            return res.status(409).json({
                message:
                "An category already exists with this name for this budget.",
            });
        }
        res.status(500).json({ message: "internal server error" });
    }
