...
describe("Category API", () => {
    ...
    test("gets a category by id", async () => {
        ...
        const response = await request(app).get(`/api/category/${categories[0]._id}`)
        expect(response.status).toBe(200)
        expect(response.body.category._id).toBe(
    ...
    test("gets all categories by budget", async () => {
        ...
        const response  = await request(app).get(`/api/category/budget/${budget1._id}`);
        expect(response.status).toBe(200);
    ...
    test("update a category", async () => {
        ...
        const response = await request(app).put(`/api/category/${categories[0]._id}`).send({
            name: "New Name"
        })
        expect(response.status).toBe(200);
    ...
    test("delete a category", async () => {
        const {categories} = await createTestCategory();
        const response = await request(app).delete(`/api/category/${categories[0]._id}`);
        expect(response.status).toBe(200);
