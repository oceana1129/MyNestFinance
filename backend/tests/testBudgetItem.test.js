...
describe("Budget Item API", () => {
    ...
    test("get item by id", async () => {
        ...
        expect(response.status).toBe(200);
        expect(response.body.item._id).toBe(
    ...
    test("get all items by a category", async () => {
        ...
        const response = await request(app).get(`/api/item/category/${category1._id}`);
        expect(response.status).toBe(200);
    ...
    test("update an item", async () => {
        ...
        const response = await request(app).put(`/api/item/${items[0]._id}`)
        .send({name: "New Name"})
        expect(response.status).toBe(200);
